...
def nearest_value(array,value):
    ...
    return idx
df=pd.read_csv("tables/hourly load per node.csv",index_col=range(4))
dfp=pd.read_csv("tables/load highlights per node.csv",index_col=range(2),header=range(2))
...
for index, k in enumerate(df.columns.tolist()):
    ...
    name='Yearly variation, medium escenary, in node {0}'.format(k)
    plt.ylabel('Load (MW)')
    plt.title(name)
...
name='Daily load variation, medium escenary, in {0} at {1},{2}'.format(k,a,m)
print dfp.xs([a, m])[k]
plt.ylabel('Load')
plt.xlabel('Hour')
plt.title(name)
...
plt.ylabel('Load (MW)')
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
plt.title('Highligh days, medium escenary, in {0} at {1}/{2}'.format(k,m,a))
plt.show()
